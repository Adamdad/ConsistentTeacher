...
def prepare_coco_data(seed=1, percent=10.0, version=2017, seed_offset=0):
    ...
    def _save_anno(name, images, annotations):
        ...
        with open(
            "{root}/{folder}/{save_name}.json".format(
                save_name=name, root=SAVE_DIR, folder="semi_supervised"
            ),
            "w",
        ) as f:
    ...
    if not os.path.exists(unlabeled_ann_file):
        ...
        print(">> Data {} saved({} images {} annotations)".format(unlabeled_ann_file,len(unlabeled_info["images"]),len(unlabeled_info["annotations"])))
        json.dump(unlabeled_info,open(os.path.join(SAVE_DIR, "instances_unlabeled{}.json".format(version)),'w'))
